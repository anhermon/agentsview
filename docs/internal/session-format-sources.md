...
- **Upstream:** Cursor's public GitHub organization and first-party docs were
  ...
  extension states it does not decode Cursor's DB-only chat blobs, and
  `ctx` (`https://github.com/ctxrs/ctx`, checked at `8c6d670`) reads only
  `agent-transcripts` and contains none of those three strings. Evidence here
  is instead direct inspection of a live local `state.vscdb` (macOS,
  86MB, `PRAGMA quick_check` = `ok`): `composerData` documents observed at
  `_v: 16` and bubble documents at `_v: 3`. `composerData.createdAt` and
...
- **Usage and cost:** No per-message or per-session token, cache, reasoning,
  ...
  documents. Agentsview emits no usage events for this agent; cost is unpriced.
- **Agentsview:** `internal/parser/cursor_ide.go` and
  ...
  avoids hashing the full database (86MB+ locally, 500MB+ reported in the
  wild): a container-level fingerprint uses whole-file size and composite
  mtime, and a member-level fingerprint uses a cheap digest over
  `lastUpdatedAt` and header count rather than full bubble content.
